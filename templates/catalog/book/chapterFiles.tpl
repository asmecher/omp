{**
 * templates/catalog/book/bookFilesContentAndDownload.tpl
 *
...
{foreach from=$availableFiles[$publicationFormatId] item=availableFile}
	{if $availableFile->getData('chapterId') == $chapterId}
		<li>
			<div class="publicationFormatLink pkp_helpers_align_left">
				...
				{strip}
					<a href="{$downloadUrl}">
						...
						{else}
							<span title="{translate key="monograph.accessLogoOpen.altText"}" class="sprite openaccess"></span>
						{/if}
					</a>
				{/strip}
			</div>
